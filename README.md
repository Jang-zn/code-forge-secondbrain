...
`~/.claude/projects/` 폴더의 JSONL 파일을 감시하다가 대화가 끝나면 AI로 요약을 생성하고, Obsidian vault에 구조화된 마크다운 노트로 저장하며 기존 문서들과 `[[wikilinks]]`로 연결합니다.

요약 엔진으로 **로컬 Claude Code CLI**(기본, 추가 비용 없음) 또는 **Gemini API**(별도 API 키 필요)를 선택할 수 있습니다.
...
~/.claude/projects/**/*.jsonl 변경 감지
  ...
  → Claude CLI 또는 Gemini API로 요약 생성 (JSON 구조화, 주제별 1-3개 분리)
...
2. (Gemini 모드일 때만) Gemini API 키 입력

끝. 이후 Claude Code 대화가 자동으로 쌓입니다.

## 요약 엔진 선택

`Cmd+,` → "secondbrain" 검색 → **Summary Provider** 항목에서 선택.

### Claude CLI (기본값, 권장)

로컬에 설치된 Claude Code CLI를 사용합니다. 추가 API 키나 과금 없이 기존 Claude 구독으로 동작합니다.

```json
{
  "secondbrain.summaryProvider": "claude-cli",
  "secondbrain.claudeCliModel": "sonnet"
}
```
...
|------|------|
| `sonnet` | 속도/품질 균형 (기본값) |
| `opus` | 최고 품질, 느림 (Max 구독 필요) |
| `haiku` | 빠르고 저렴, 낮은 품질 |

설정 화면의 "🔗 연결 테스트" 링크로 동작 확인 가능.

> nvm 등으로 `claude` 경로가 안 잡히면 `secondbrain.claudeCliBinary`에 절대 경로 입력.
> (`which claude`로 확인)
...
```json
{
  "secondbrain.summaryProvider": "gemini",
...
| 커맨드 | 설명 |
|---|---|
| `SecondBrain: Setup (Vault + API Key)` | **첫 실행 시** — vault 경로 설정 (Gemini 모드이면 API 키도 입력) |
| `SecondBrain: Set Vault Path` | vault 경로만 재설정 |
| `SecondBrain: Set Gemini API Key` | Gemini API 키만 재설정 |
| `SecondBrain: AI 연결 테스트` | 현재 provider 연결 상태 확인 |
...
├── summarizer/
│   ├── types.ts               # SummaryResult + Summarizer 인터페이스
│   ├── ClaudeCLISummarizer.ts # claude CLI spawn → 구조화된 JSON 요약
│   └── GeminiSummarizer.ts    # Gemini API → 구조화된 JSON 요약
├── vault/
